...
const CopyableSettingItem: React.FC<{
  ...
  const menuItems = [
    ...
    ...(Array.isArray(value)
      ...
      : prefix.length === 1 ? [
        ...(prefix[0] === 'DOMAIN-SUFFIX'
          ? getSubDomains(value as string).map((v) => ({
            key: `${prefix[0]},${v}`,
            text: `${prefix[0]},${v}`
          }))
          : prefix[0] === 'IP-ASN' || prefix[0] === 'SRC-IP-ASN'
            ? [{
              key: `${prefix[0]},${(value as string).split(' ')[0]}`,
              text: `${prefix[0]},${(value as string).split(' ')[0]}`
            }]
            : [{
              key: `${prefix[0]},${value}${(prefix[0] === 'IP-CIDR' || prefix[0] === 'SRC-IP-CIDR') ? (isIPv6(value as string) ? '/128' : '/32') : ''}`,
              text: `${prefix[0]},${value}${(prefix[0] === 'IP-CIDR' || prefix[0] === 'SRC-IP-CIDR') ? (isIPv6(value as string) ? '/128' : '/32') : ''}`
            }])
      ] : [])
  ]
